...
int main(int argc, char *argv[]) {
    ...
    // order P kernel requires P-1 ghost slabs on EACH side of the local domain.
    auto ghost_slabs = iOrder > 1 ? iOrder - 1 : 0;
    auto slab_count = local_n0 + (2 * ghost_slabs);


    // calculate max memory required: Spatial grid (with ghosts) vs. FFTW workspace
    auto slab_floats = size_t(slab_count) * size_t(nGrid) * size_t(2*k_nz);
    auto fftw_floats = size_t(2) * size_t(complex_count);
    auto n_slab_floats = std::max(slab_floats, fftw_floats);

    float *slab_data = new (std:: align_val_t (64)) float [n_slab_floats]; // 512-bit alignment


    // create the base view and shift the coordinate system leftward by the ghost radius
    ...
    // ghost view spans the entire allocated spatial memory
    Array<float,3> ghost(raw_slab(
        Range(local_0_start - ghost_slabs, local_0_start + local_n0 - 1 + ghost_slabs),
        Range(0, nGrid-1), 
        Range(0, nGrid-1)
    ));
    ...
    // k-space view strictly maps the unpadded domain
    ...
    float initial_mass_local = blitz::sum(m2);
    float initial_mass_total;
    MPI_Allreduce(&initial_mass_local, &initial_mass_total, 1, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);
    if (irank == 0) {
        std::cerr << "Initial Particle Mass: " << initial_mass_total << std::endl;
        std::cerr << "Final Grid Mass:       " << total_mass << std::endl;
        std::cerr << "Leakage:               " << std::abs(initial_mass_total - total_mass) << std::endl;
    }
