...
int main(int argc, char *argv[]) {
    ...
    // n0 = nGrid, n1 = nGrid
    // howmany = 2 * k_nz (the number of floats in the z-pencil)
    ptrdiff_t local_n0, local_0_start, local_n1, local_1_start;
    
    ptrdiff_t n_dims[2] = {nGrid, nGrid}; 

    ptrdiff_t alloc_local_floats = fftwf_mpi_local_size_many_transposed(
        2, n_dims, 2 * k_nz,
    ...
    auto float_count = std::max((ptrdiff_t)(slab_count * nGrid * 2 * k_nz), alloc_local_floats);
    ...
    storage.ordering() = 2, 0, 1; 
    storage.base() = 0, local_1_start, 0;

    Array<std::complex<float>,3> kslab(
        reinterpret_cast<std::complex<float>*>(slab_data),
        shape(nGrid, local_n1, k_nz),
        storage
    );
    ...
    auto plan_2d = fftwf_plan_dft_r2c_2d(nGrid, nGrid,
            slab.dataFirst(),
            reinterpret_cast<fftwf_complex*>(slab_data),
            FFTW_ESTIMATE);

    auto plan_transpose = fftwf_mpi_plan_many_transpose(
        nGrid, nGrid, 2 * k_nz,
        FFTW_MPI_DEFAULT_BLOCK, FFTW_MPI_DEFAULT_BLOCK,
        slab_data, slab_data, 
    ...
    for(int i = 0; i < local_n0; ++i) {
        // real arrays have padding, so the stride is nGrid * 2 * k_nz
        float* in = slab.dataFirst() + i * (nGrid * 2 * k_nz);
        
        // complex arrays don't have the padding, stride is nGrid * k_nz
        fftwf_complex* out = reinterpret_cast<fftwf_complex*>(slab_data) + i * (nGrid * k_nz);
        fftwf_execute_dft_r2c(plan_2d, in, out);
    }
    fftwf_destroy_plan(plan_2d);

    // 2. manual network transpose (X, Y, Z -> Y, X, Z)
